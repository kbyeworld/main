...
class marble_play(commands.Cog):
    ...
    async def play_start(self, ctx,
        ...
        mydict[ctx.author.id] = {"channel_id": thread.id, "players": [ctx.author.id],
        ...
        try:
            await start_msg.edit(content=f"✅ {'이 서버에서' if kind == 'Server' else '글로벌 멀티'} 게임이 시작 준비중이에요.", embed=embed,
                                 view=view)
